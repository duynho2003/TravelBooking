...
@AllArgsConstructor
@RestController
@RequestMapping("/api/v1/tourRoomBookings")
public class TourRoomBookingController {

    private TourRoomBookingService tourRoomBookingService;

    @PreAuthorize("hasAuthority('CREATE_TOUR')")
    @PostMapping("/create")
    public ResponseEntity<?> createTourRoomBooking(@RequestBody CreateTourRoomBookingRequestDto createTourRoomBookingRequestDto) {
        try {
            CreateTourRoomBookingResponseDto responseDto = tourRoomBookingService.createTourRoomBooking(createTourRoomBookingRequestDto);
