...
class CheckoutSuccess extends StatefulWidget {
  ...
  const CheckoutSuccess({super.key, this.bookingData});
...
class _CheckoutSuccessState extends State<CheckoutSuccess> {
  ...
  Widget _buildOrderDetails(Map<String, dynamic>? bookingData) {
    if (bookingData == null) {
      return Container(); // Handle the case where no data is passed
    }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          _buildOrderDetailRow('Order Number', bookingData['code'] ?? 'N/A'),
          _buildOrderDetailRow(
              'Total Amount', formatCurrency(bookingData['amount']) ?? 'N/A'),
          _buildOrderDetailRow(
